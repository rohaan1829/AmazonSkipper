			...
			<ul
				className={`nav flex items-center  ${
					headerType === 5
						...
						? `gap-x-3 lg:gap-x-4 xl:gap-x-6 2xl:gap-x-35px lg:px-6 xl:px-10 rounded-full ${
								headerType === 6 || headerType === 9
									? "lg:bg-cream-light-color lg:dark:bg-black-color"
									: "lg:border lg:border-primary-color "
						  } `
						: ` gap-x-5 lg:gap-x-6 2xl:gap-x-45px`
				}`}
			>
				{filteredNavItems?.length
					? filteredNavItems?.map(({ name, path, path2 }, idx) => (
							...
							className={`text-sm lg:text-size-15 font-medium  ${
									...
									: headerType === 5 ||
										...
										headerType === 5 || headerType === 6 || headerType === 9
											? ` text-[#22C55E] hover:text-[#15803D] dark:text-[#22C55E] dark:hover:text-[#16A34A] ${
													headerType === 6 || headerType === 9
														? "py-2 md:py-12px lg:py-12px xl:py-14px 2xl:py-14px"
														: "px-2 lg:px-12px xl:px-14px py-1 lg:py-3px border border-transparent hover:border-primary-color rounded-full"
											  }   `
											: headerType === 4
											? ` text-[#22C55E] hover:text-[#15803D] dark:text-[#22C55E] dark:hover:text-[#16A34A] md:py-12px lg:py-12px xl:py-14px 2xl:py-14px `
											: `${
													headerType === 10
														...
														: "text-[#22C55E] hover:text-[#15803D] dark:text-[#22C55E] dark:hover:text-[#16A34A] md:py-12px lg:py-20px xl:py-25px 2xl:py-30px  after:w-0 after:h-0.5 after:bg-gradient-primary after:absolute after:right-0 hover:after:left-0 after:bottom-[25px] after:transition-all after:duration-500 group-hover:after:w-full"
											  }`
									} `}
								>
									{name}
