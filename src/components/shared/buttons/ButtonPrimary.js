...
const ButtonPrimary = ({ children, type, url, className, isIcon }) => {
	return (
		<Link
			href={url ? url : "#"}
			className={`text-sm lg:text-size-15 font-bold text-white-color capitalize py-3 lg:py-17px px-6 lg:px-8 xl:px-35px ${
				type === 2 ? "" : "ml-10px"
			} bg-200 bg-gradient-secondary hover:bg-[-100%] rounded-full leading-1 ${className} ${
				isIcon ? "inline-flex gap-2 lg:gap-10px items-center " : ""
